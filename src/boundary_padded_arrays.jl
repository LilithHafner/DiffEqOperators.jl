...
abstract type AbstractDirectionalBoundaryPaddedArray{T, N, D} <:
              AbstractBoundaryPaddedArray{T, N} end
...
function compose(padded_arrays::BoundaryPaddedArray...)
    ...
    (length(padded_arrays) == N) ||
        throw(ArgumentError("The padded_arrays must cover every dimension - make sure that the number of padded_arrays is equal to ndims(u)."))
    for D in Ds
