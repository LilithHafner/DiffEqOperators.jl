...
function LinearAlgebra.Array(Q::AffineBC{T}, N::Int) where {T}
    Q_L = [transpose(Q.a_l) transpose(zeros(T, N - length(Q.a_l))); Diagonal(ones(T, N));
...
function SparseArrays.SparseMatrixCSC(Q::AffineBC{T}, N::Int) where {T}
    ...
    a_r = Q.a_r[findfirst(!iszero, Q.a_r) isa Nothing ? something((end + 1):end) :
                something(findfirst(!iszero, Q.a_r):end)]
    a_l = Q.a_l[findfirst(!iszero, Q.a_l) isa Nothing ? something((end + 1):end) :
                something(findfirst(!iszero, Q.a_l):end)]
    for (j, e) in enumerate(a_l)
        BandedMatrices.inbands_setindex!(Q_l, e, 1, j)
    end
    for (j, e) in enumerate(a_r)
...
function LinearAlgebra.Array(A::DerivativeOperator{T, N, true},
    ...
    for i in (bpc + 1):(len - bpc - offside)
        ...
        cur_stencil = cur_coeff >= 0 ? cur_stencil :
                      ((-1)^A.derivative_order) * reverse(cur_stencil)
        if cur_coeff >= 0
            L[i, (i + 1 - offside):(i + stl - offside)] = cur_coeff * cur_stencil
        else
    ...
    for i in 1:(bpc + offside)
        ...
        if cur_coeff < 0 && (len + 2 - stl - bpc + i >= 1) && (i <= bpc + 1)
            ...
            L[len - bpc + i - offside, (i + len - stl - bpc + 2):(i + len - bpc + 1)] = cur_coeff *
                                                                                        cur_stencil
        ...
        elseif cur_coeff >= 0 && i < offside + 1
            cur_stencil = stencils
            L[len - bpc + i - offside, (len - stl + i - offside + 3):(len + i - offside + 2)] = cur_coeff *
                                                                                                cur_stencil
        else
...
function LinearAlgebra.Array(A::DerivativeOperator{T, N, true, M},
    ...
    for i in 1:bpc
        ...
        elseif cur_coeff >= 0 && i >= offside
            L[i, (i + 1 - offside):(i + stl - offside)] = cur_coeff *
                                                          A.low_boundary_coefs[1, i]
        else
...
function Array(L::DiffEqOperatorComposition, s)
    prod(Array.(reverse(L.ops), fill(s, length(L.ops))))
end
