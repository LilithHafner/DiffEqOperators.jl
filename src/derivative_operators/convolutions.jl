...
function convolve_interior!(x_temp::AbstractVector{T1}, x::AbstractVector{T2},
    ...
    if !add_range
        ...
        for i in [(1 + A.boundary_point_count):(A.boundary_point_count + offset);
                  (len - A.boundary_point_count - offset + 1):(len - A.boundary_point_count)]
            xtempi = zero(T)
            cur_stencil = eltype(stencil) <: AbstractVector ?
                          stencil[i - A.boundary_point_count] : stencil
            cur_coeff = coeff[i]
            for idx in 1:(A.stencil_length)
                xtempi += cur_coeff * cur_stencil[idx] * x[i - mid + idx]
            end
...
function convolve_interior!(x_temp::AbstractVector{T}, x::AbstractVector{T},
    ...
    for i in (1 + A.boundary_point_count):(length(x_temp) - A.boundary_point_count - A.offside)
        ...
        cur_stencil = stencil
        cur_coeff = coeff[i]
        cur_stencil = cur_coeff >= 0 ? cur_stencil :
                      A.derivative_order % 2 == 0 ? reverse(cur_stencil) :
                      -1 * reverse(cur_stencil)
        for idx in 1:(A.stencil_length)
            x_idx = cur_coeff < 0 ? x[i - A.stencil_length + 1 + idx + A.offside] :
                    x[i + idx - A.offside]
    ...
    for i in 1:(A.boundary_point_count + A.offside)
        ...
        if cur_coeff < 0 && _x_len - A.stencil_length - A.boundary_point_count + i >= 1 &&
           i <= A.boundary_point_count + 1
            ...
            for idx in 1:(A.stencil_length)
                xtempi += cur_coeff * cur_stencil[idx] *
        ...
        elseif cur_coeff >= 0 && i < A.offside + 1
            ...
            for idx in 1:(A.boundary_stencil_length)
                xtempi += cur_coeff * cur_stencil[idx] *
                          x[_x_len - A.boundary_stencil_length + idx]
            end
        end
        x_temp[x_temp_len - A.boundary_point_count + i - A.offside] = xtempi +
                                                                      !overwrite *
...
function convolve_BC_left!(x_temp::AbstractVector{T1}, _x::BoundaryPaddedVector,
    ...
    xtempi = zero(T)
    cur_stencil = eltype(A.stencil_coefs) <: AbstractVector ?
...
function convolve_interior!(x_temp::AbstractVector{T}, _x::BoundaryPaddedVector,
    ...
    for i in (1 + A.boundary_point_count):(length(x_temp) - A.boundary_point_count - A.offside)
        ...
        cur_stencil = cur_coeff >= 0 ? cur_stencil :
                      ...
                      -1 * reverse(cur_stencil)
        for idx in 1:(A.stencil_length)
            index = cur_coeff < 0 ? i - A.stencil_length + 1 + idx + A.offside :
                    i + idx - A.offside
            if index == _x_len
                x_idx = _x.r
            elseif index == 1
    ...
    for i in 1:(A.boundary_point_count + A.offside)
        ...
        if cur_coeff < 0 && _x_len - A.stencil_length - A.boundary_point_count + i >= 1 &&
           i <= A.boundary_point_count + 1
            ...
            cur_stencil = ((-1)^A.derivative_order) * reverse(cur_stencil)
            for idx in 1:(A.stencil_length)
                x_idx = _x_len - A.stencil_length + idx - A.boundary_point_count + i - 1 ==
                        _x_len ? _x.r :
        ...
        elseif cur_coeff < 0 &&
            ...
            cur_stencil = ((-1)^A.derivative_order) * reverse(cur_stencil)
            for idx in 1:(A.boundary_stencil_length)
                x_idx = _x_len - A.boundary_stencil_length + idx == _x_len ? _x.r :
        ...
        elseif cur_coeff >= 0 && i < A.offside + 1
            cur_stencil = downwind_stencils
            for idx in 1:(A.stencil_length)
                x_idx = _x_len - A.stencil_length + idx - A.offside + i == _x_len ? _x.r :
...
function convolve_interior!(x_temp::AbstractVector{T}, _x::BoundaryPaddedVector,
    ...
    for i in (bpc + 1):(len - bpc - A.offside)
        ...
        if cur_coeff >= 0
            ...
            cur_stencil = A.stencil_coefs[1, i - bpc]
            for idx in 1:stl
                x_idx = i + idx - A.offside == _x_len ? _x.r :
            ...
            cur_stencil = A.stencil_coefs[2, i - bpc]
            for idx in 1:stl
                x_idx = i - stl + 1 + idx + A.offside > 1 ? x[i - stl + idx + A.offside] :
                        _x.l
                xtempi += cur_coeff * cur_stencil[idx] * x_idx
            end
            x_temp[i] = xtempi + !overwrite * x_temp[i]
