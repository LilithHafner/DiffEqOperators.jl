...
function stencil(q::PeriodicBC{T}, N::Int) where {T}
    ([transpose(zeros(T, N - 1)) one(T)], [one(T) transpose(zeros(T, N - 1))])
end
...
function NeumannBC(α::NTuple{2, T}, dx::Union{AbstractVector{T}, T}, order = 1) where {T}
    RobinBC((zero(T), one(T), α[1]), (zero(T), one(T), α[2]), dx, order)
end
function DirichletBC(αl::T, αr::T) where {T}
    RobinBC((one(T), zero(T), αl), (one(T), zero(T), αr), one(T), 2one(T))
end
#specialized constructors for Neumann0 and Dirichlet0
Dirichlet0BC(T::Type) = DirichletBC(zero(T), zero(T))
function Neumann0BC(dx::Union{AbstractVector{T}, T}, order = 1) where {T}
    NeumannBC((zero(T), zero(T)), dx, order)
end
