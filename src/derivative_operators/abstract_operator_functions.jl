...
function checkbounds(A::AbstractDerivativeOperator, k::Integer, j::Integer)
    (0 < k ≤ size(A, 1) && 0 < j ≤ size(A, 2) || throw(BoundsError(A, (k, j))))
end

function checkbounds(A::AbstractDerivativeOperator, kr::AbstractRange, j::Integer)
    (checkbounds(A, first(kr), j); checkbounds(A, last(kr), j))
end

function checkbounds(A::AbstractDerivativeOperator, k::Integer, jr::AbstractRange)
    (checkbounds(A, k, first(jr)); checkbounds(A, k, last(jr)))
end

function checkbounds(A::AbstractDerivativeOperator, kr::AbstractRange, jr::AbstractRange)
    (checkbounds(A, kr, first(jr)); checkbounds(A, kr, last(jr)))
end

function checkbounds(A::AbstractDerivativeOperator, k::Colon, j::Integer)
    (0 < j ≤ size(A, 2) || throw(BoundsError(A, (size(A, 1), j))))
end

function checkbounds(A::AbstractDerivativeOperator, k::Integer, j::Colon)
