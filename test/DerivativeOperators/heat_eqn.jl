...
@testset "Parabolic Heat Equation with Dirichlet BCs" begin
    ...
    step = (u, p, t) -> A * bc * u
    heat_eqn = ODEProblem(step, u0, (0.0, 1.0))
...
@testset "Parabolic Heat Equation with Robin BCs" begin
    ...
    for t in 0.2:0.1:1.0
        @test params[1] * soln(t)[1] -
              params[2] * sum(first_order_coeffs_start .* soln(t)[1:4])≈left_RBC atol=4e-1
        # append!(val,params[1]*soln(t)[1] + -params[2]*sum(first_order_coeffs_start .* soln(t)[1:4]) - left_RBC)
        @test params[1] * soln(t)[end] +
              params[2] * sum(first_order_coeffs_end .* soln(t)[(end - 3):end])≈right_RBC atol=4e-1
    end
