...
@test_skip begin
    @testset "Test Constructor, Multiplication, and Concretization" begin
        ...
        L1 = UpwindDifference(1, 1, dx, N, 1.0)
        ...
        c = rand(N)
        L = CenteredDifference(4, 4, dx, N)
        L1 = UpwindDifference(1, 1, 1.0, N, 1.0)
        A1 = L1 * Q
        cA = c * A
        cL = c * L
        cLQ = (c * L) * Q
        ...
        u = rand(N + 2)
        @test (L + L2) * u ≈ convert(AbstractMatrix, L + L2) * u ≈
              (BandedMatrix(L) + BandedMatrix(L2)) * u
    ...
    @testset "Test Constructor, Multiplication, and Concretization (Non-uniform grid)" begin
        ...
        Q = RobinBC((al, bl, cl), (ar, br, cr), dx)
        L = UpwindDifference(1, 1, dx, N + 1, 1.0)
        ...
        @test Array(A)[1] ≈
              (Array(L) * Array(Q, N + 1)[1], Array(L) * Array(Q, N + 1)[2])[1]
        @test Array(A)[2] ≈
    ...
    @testset "Test Left Division L4 (fourth order)" begin
        ...
        @test norm(analytic_AL \ u_translated - As_l \ u_translated) /
              norm(analytic_AL \ u_translated) < cond(analytic_AL) * eps()
        ...
        @test_broken analytic_M = analytic_Am \
                                  (reshape(M2, prod(s)) .- repeat(analytic_Ab, 3))
